...
function checkDirection() {
    if((window.innerHeight < window.innerWidth) && isMobile(navigator.userAgent || navigator.vendor || window.opera)){
        alert("Please use Portrait mode, or play on a computer!", () => {
            checkDirection();
        });
    }
}
...
document.getElementsByTagName("canvas")[0].addEventListener("pointerdown", () => {
    window.isBeingClicked = true;
})
document.getElementsByTagName("canvas")[0].addEventListener("pointerup", () => {
    window.isBeingClicked = false;
})